...
void manualCalibrate() {
  ...
  if (maxIntensity == redIntensity) {
    for (int brightness = 0; brightness <= 255; brightness++) {
      ...
      if (redIntensity - blueIntensity <= 10 && redIntensity - greenIntensity <= 10) {
        lcd.print("done");
        break;
      } else {
        analogWrite(greenLEDPin, brightness);
  ...
  if (maxIntensity == blueIntensity) {
    for (int brightness = 0; brightness <= 255; brightness++) {
      ...
      blueIntensity = analogRead(LDRPin);
      analogWrite(redLEDPin, 0);
      if (blueIntensity - redIntensity <= 10 && blueIntensity - greenIntensity <= 10) {
        lcd.print("done");
        break;
      } else {
        analogWrite(greenLEDPin, brightness);
  ...
  if (maxIntensity == greenIntensity) {
    for (int brightness = 0; brightness <= 255; brightness++) {
      analogWrite(redLEDPin, brightness);
      redIntensity = analogRead(LDRPin);
      greenIntensity = analogRead(LDRPin);
      ...
      analogWrite(redLEDPin, 0);
      // analogWrite(blueLEDPin, brightness);
      if (greenIntensity - redIntensity <= 10 && greenIntensity - blueIntensity <= 10) {
        lcd.print("done");
        lcd.print("Wohoooooo");
        break;
      } else {
